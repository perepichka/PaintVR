...
#endif
};FluvioVertexOutputForwardBase FluvioVertForwardBase (FluvioVertexInput v){FluvioVertexOutputForwardBase o;UNITY_INITIALIZE_OUTPUT(FluvioVertexOutputForwardBase, o);if (_CullFluid < 0){o.pos = 0;}else {float4 posWorld = mul(_Object2World, v.vertex);
#if UNITY_SPECCUBE_BOX_PROJECTION
o.posWorld = posWorld.xyz;
#endif
float3 objPosWorld = mul(_Object2World, float4(0,0,0,1));float3 dir = normalize(objPosWorld - _WorldSpaceCameraPos) * _Fluvio_DepthPassCellSpace;float3 nearPoint = objPosWorld + dir;float3 farPoint = objPosWorld - dir;float nearDepth = -mul(UNITY_MATRIX_VP, float4(nearPoint, 1)).z;float farDepth = -mul(UNITY_MATRIX_VP, float4(farPoint,1)).z;float depth = abs(invlerp(nearDepth, farDepth, -mul(UNITY_MATRIX_MVP, v.vertex).z)) * _Fluvio_DepthPassCellSpace;if (((depth / _Fluvio_DepthPassCellSpace % _Fluvio_DepthPassCount) - _Fluvio_DepthPassIndex) > FLUVIO_DEPTHPASS_BUCKET_SIZE){o.pos = 0;}else {o.pos = mul(UNITY_MATRIX_MVP, v.vertex);o.tex = FluvioTexCoords(v);o.eyeVec = NormalizePerVertexNormal(posWorld - _WorldSpaceCameraPos);float3 normalWorld = FluvioGetWorldNormal(v.normal);
#ifdef _TANGENT_TO_WORLD
...
#endif
UNITY_APPLY_FOG(i.fogCoord, c.rgb);return FluvioOutputForward (c, s.alpha);}struct FluvioVertexOutputForwardAdd{float4 pos       : SV_POSITION;float2 tex       : TEXCOORD0;half3 eyeVec       : TEXCOORD1;half4 tangentToWorldAndLightDir[3] : TEXCOORD2;LIGHTING_COORDS(5,6)UNITY_FOG_COORDS(7)};FluvioVertexOutputForwardAdd FluvioVertForwardAdd (FluvioVertexInput v){FluvioVertexOutputForwardAdd o;UNITY_INITIALIZE_OUTPUT(FluvioVertexOutputForwardAdd, o);if (_CullFluid < 0){o.pos = 0;}else {float4 posWorld = mul(_Object2World, v.vertex);o.pos = mul(UNITY_MATRIX_MVP, v.vertex);o.tex = FluvioTexCoords(v);o.eyeVec = NormalizePerVertexNormal(posWorld.xyz - _WorldSpaceCameraPos);float3 normalWorld = FluvioGetWorldNormal(v.normal);
#ifdef _TANGENT_TO_WORLD
...
#endif
};FluvioVertexOutputDeferred FluvioVertDeferred (FluvioVertexInput v){FluvioVertexOutputDeferred o;UNITY_INITIALIZE_OUTPUT(FluvioVertexOutputDeferred, o);if (_CullFluid < 0){o.pos = 0;}else {float4 posWorld = mul(_Object2World, v.vertex);
#if UNITY_SPECCUBE_BOX_PROJECTION
o.posWorld = posWorld;
#endif
o.pos = mul(UNITY_MATRIX_MVP, v.vertex);float3 objPosWorld = mul(_Object2World, float4(0, 0, 0, 1));float3 dir = normalize(objPosWorld - _WorldSpaceCameraPos) * _Fluvio_DepthPassCellSpace;float3 nearPoint = objPosWorld + dir;float3 farPoint = objPosWorld - dir;float nearDepth = -mul(UNITY_MATRIX_VP, float4(nearPoint, 1)).z;float farDepth = -mul(UNITY_MATRIX_VP, float4(farPoint, 1)).z;float depth = abs(invlerp(nearDepth, farDepth, -mul(UNITY_MATRIX_MVP, v.vertex).z)) * _Fluvio_DepthPassCellSpace;if (((depth / _Fluvio_DepthPassCellSpace % _Fluvio_DepthPassCount) - _Fluvio_DepthPassIndex) > FLUVIO_DEPTHPASS_BUCKET_SIZE){o.pos = 0;}else {o.tex = FluvioTexCoords(v);o.eyeVec = NormalizePerVertexNormal(posWorld.xyz - _WorldSpaceCameraPos);float3 normalWorld = FluvioGetWorldNormal(v.normal);
#ifdef _TANGENT_TO_WORLD
float4 tangentWorld = FluvioGetWorldTangent(v.tangent);float3x3 tangentToWorld = CreateTangentToWorldPerVertex(normalWorld, tangentWorld.xyz, tangentWorld.w);o.tangentToWorldAndColor[0].xyz = tangentToWorld[0];o.tangentToWorldAndColor[1].xyz = tangentToWorld[1];o.tangentToWorldAndColor[2].xyz = tangentToWorld[2];
#else 
o.tangentToWorldAndColor[0].xyz = 0;o.tangentToWorldAndColor[1].xyz = 0;o.tangentToWorldAndColor[2].xyz = normalWorld;
